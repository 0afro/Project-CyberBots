...
@onready var objective_text = $ObjectiveLabel if has_node("ObjectiveLabel") else null
...
func _on_health_changed(new_health: float, max_health: float):
	# Bar always shows full 200 HP range (doesn't shrink when blocks lost)
	health_bar.max_value = 200.0
	health_bar.value = new_health
	health_value.text = str(int(new_health))
	
	# Cyan blue when healthy, red when critical (≤50 HP)
	var health_percent = new_health / 200.0
	if health_percent > 0.25:  # FIXED - now 50 HP (Block 1)
		# Healthy - Bright cyan
...
func _on_shield_changed(new_shield: float, max_shield: float):
	...
	# Shield is bright blue when active, grey when empty
	if new_shield > 0:
		shield_bar.modulate = Color(0.0, 0.8, 1.0)
	else:
		shield_bar.modulate = Color(0.3, 0.3, 0.3)

func update_objective(current: int, total: int):
	if objective_text:
		objective_text.text = "Area " + str(current) + "/" + str(total)

func show_victory():
	# Simple victory message
	if objective_text:
		objective_text.text = "VICTORY!"
		objective_text.modulate = Color.GREEN
